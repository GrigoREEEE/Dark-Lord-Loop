...
func generate_height_map(width: int, height: int, noise_seed : int, res_scale : float = 1) -> Dictionary[Vector2, float]:
	...
	detail_noise.frequency = 0.08 / res_scale 
	...
	for x in range(width):
		for y in range(height):
			...
			# We offset the Y value used for the slant using noise. 
			# This makes the north-to-south drop wavy rather than a perfect ramp.
			...
			# Using warped_ny instead of ny means the slope isn't uniform
			...
			# Add the micro-noise. We scale it down to 0.05 so it just adds 
			# subtle texture to the terrain without creating massive spikes.
